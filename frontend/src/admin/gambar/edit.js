...
const GambarEdit = () => {
    ...
    useEffect(() => {
        ...
        const fetchGambarDetail = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/picture/${id}`,{
                    headers:{
                        'Authorization' :`Bearer ${authToken}`
                    },
                });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`http://127.0.0.1:8000/api/picture-update/${id}`, formDataObj, {
                headers: {
