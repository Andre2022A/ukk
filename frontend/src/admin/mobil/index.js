...
const Mobil = () => {
    ...
    useEffect(() => {
      ...
      const fetchMobil = async () => {
        ...
        try {
          ...
          const response = await fetch('http://127.0.0.1:8000/api/mobil');
      ...
      const handleDelete = async (mobilId) => {
        ...
        if (result.isConfirmed) {
          try {
              const deleteUrl = `http://127.0.0.1:8000/api/mobil-delete/${mobilId}`;
              ...
              // Optionally, you can update the state to remove the deleted mobil
              setMobil((prevMobil) =>
                  prevMobil.filter((mobil) => mobil.id !== mobilId)
              );
    ...
    return (
        ...
        <div>
  <h1 className="h3 mb-3 text-gray-800">Data Mobil</h1>
  {/* DataTales Example */}
          ...
          {mobil.map((mobil, index) => (
            <tr key={`mobil-${index}`}>
              ...
              <td className='text-center'>
                <NavLink to={`edit/${mobil.id_mobil}`} className='mr-3'>
                    <i className="bi bi-pen-fill text-warning"></i>
                </NavLink>
                |
                <a role='button' className='ml-3' onClick={() => handleDelete(mobil.id_mobil)}>
